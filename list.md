...
Test_Loop                                 | cmd      |  | "FCVAR_CHEAT"    | Test_Loop <loop name> - loop back to the specified loop start point unconditionally.
...
cl_find_ent                               | cmd      |  | "FCVAR_CLIENTDLL", "FCVAR_CHEAT" | Find and list all client entities with classnames that contain the specified substring. Format: cl_find_ent <substring> 
cl_find_ent_index                         | cmd      |  | "FCVAR_CLIENTDLL", "FCVAR_CHEAT" | Display data for clientside entity matching specified index. Format: cl_find_ent_index <index> 
...
cl_pred_track                             | cmd      |  | "FCVAR_CLIENTDLL" | <entindex> <fieldname>:  Track changes to entity index entindex, for field fieldname.
...
commentary_showmodelviewer                | cmd      |  | "FCVAR_CLIENTDLL" | Display the commentary model viewer. Usage: commentary_showmodelviewer <model name> <optional attached model name>
...
dbghist_addline                           | cmd      |  | "FCVAR_GAMEDLL"  | Add a line to the debug history. Format: <category id> <line>
dbghist_dump                              | cmd      |  | "FCVAR_GAMEDLL"  | Dump the debug history to the console. Format: <category id>     Categories:      0: Entity I/O      1: AI Decisions      2: Scene Print      3: Alyx Blind      4: Log of damage done to player
...
ent_dump                                  | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Usage:    ent_dump <entity name> 
ent_fire                                  | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Usage:    ent_fire <target> [action] [value] [delay] 
ent_info                                  | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Usage:    ent_info <class name> 
ent_keyvalue                              | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Applies the comma delimited key=value pairs to the entity with the given Hammer ID. 	Format: ent_keyvalue <entity id> <key1>=<value1>,<key2>=<value2>,...,<keyN>=<valueN> 
...
ent_orient                                | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Orient the specified entity to match the player's angles. By default, only orients target entity's YAW. Use the 'allangles' option to orient on all axis. 	Format: ent_orient <entity name> <optional: allangles>
...
ent_teleport                              | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Teleport the specified entity to where the player is looking. 	Format: ent_teleport <entity name>
...
explodevector                             | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_PRINTABLEONLY" | Kills a player applying an explosive force. Usage: explodevector <player> <x value> <y value> <z value>
...
find_ent                                  | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Find and list all entities with classnames or targetnames that contain the specified substring. Format: find_ent <substring> 
find_ent_index                            | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Display data for entity matching specified index. Format: find_ent_index <index> 
...
global_set                                | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | global_set <globalname> <state>: Sets the state of the given env_global (0 = OFF, 1 = ON, 2 = DEAD).
...
groundlist                                | cmd      |  | "FCVAR_GAMEDLL"  | Display ground entity list <index>
...
hidepanel                                 | cmd      |  | "FCVAR_CLIENTDLL" | Hides a viewport panel <name>
...
mp_backup_round_file_pattern              | convar   | %prefix%_round%round%.txt | "FCVAR_GAMEDLL", "FCVAR_RELEASE" | If set then server will save all played rounds information to files named by this pattern, e.g.'%prefix%_%date%_%time%_%team1%_%team2%_%map%_round%round%_score_%score1%_%score2%.txt'
...
mp_guardian_loc_adjective                 | convar   |          | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | (If set) kill condition token (#quest_hud_guardian_adjective_<name>)
mp_guardian_loc_condition                 | convar   |          | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | (If set) kill condition token (#quest_hud_guardian_condition_<name>)
mp_guardian_loc_icon                      | convar   |          | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | (If set) icon to override for guardian mission
mp_guardian_loc_mission                   | convar   | default  | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | Token to use to generate guardian mission (#quest_hud_guardian_mission_<name>)
mp_guardian_loc_override                  | convar   |          | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | Token to use to display guardian mission (#quest_hud_guardian_override_<name>) (if exists)
mp_guardian_loc_weapon                    | convar   |          | "FCVAR_GAMEDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | (If set) weapon name token (#SFUI_WPNHUD_<name>)
...
net_showpeaks                             | convar   | 0        | "FCVAR_DEVELOPMENTONLY" | Show messages for large packets only: <size>
...
playvideo                                 | cmd      |  | "FCVAR_CLIENTDLL" | Plays a video: <filename> [width height]
playvideo_end_level_transition            | cmd      |  | "FCVAR_CLIENTDLL" | Plays a video fullscreen without ability to skip (unless dev 1) and fades in: <filename> <time>
playvideo_exitcommand                     | cmd      |  | "FCVAR_CLIENTDLL" | Plays a video and fires and exit command when it is stopped or finishes: <filename> <exit command>
playvideo_exitcommand_nointerrupt         | cmd      |  | "FCVAR_CLIENTDLL" | Plays a video (without interruption) and fires and exit command when it is stopped or finishes: <filename> <exit command>
playvideo_nointerrupt                     | cmd      |  | "FCVAR_CLIENTDLL" | Plays a video without ability to skip: <filename> [width height]
...
r_cleardecals                             | cmd      |  | "FCVAR_CLIENTCMD_CAN_EXECUTE" | Usage r_cleardecals <permanent>.
...
replay_start                              | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | Start GOTV replay: replay_start <delay> [<player name or index>]
...
rr_forceconcept                           | cmd      |  | "FCVAR_GAMEDLL", "FCVAR_CHEAT" | fire a response concept directly at a given character. USAGE: rr_forceconcept <target> <concept> "criteria1:value1,criteria2:value2,..." criteria values are optional. 
...
showinfo                                  | cmd      |  | "FCVAR_CLIENTDLL" | Shows a info panel: <type> <title> <message> [<command number>]
showpanel                                 | cmd      |  | "FCVAR_CLIENTDLL" | Shows a viewport panel <name>
...
snd_print_channel_by_guid                 | cmd      |  | "FCVAR_CHEAT"    | Prints the content of a channel from its guid. snd_print_channel_by_guid <guid>.
snd_print_channel_by_index                | cmd      |  | "FCVAR_CHEAT"    | Prints the content of a channel from its index. snd_print_channel_by_index <index>.
...
snd_set_master_volume                     | cmd      |  | "FCVAR_CHEAT"    | Sets the master volume for a channel. snd_set_master_volume <guid> <mastervolume>.
...
stopvideos_fadeout                        | cmd      |  | "FCVAR_CLIENTDLL" | Fades out all videos playing to the screen: <time>
...
sv_jump_impulse                           | convar   | 301.993377 | "FCVAR_CLIENTDLL", "FCVAR_REPLICATED", "FCVAR_RELEASE" | Initial upward velocity for player jumps; sqrt(2*gravity*height).
...
testhudanim                               | cmd      |  | "FCVAR_CLIENTDLL", "FCVAR_CHEAT" | Test a hud element animation. 	Arguments: <anim name> 
